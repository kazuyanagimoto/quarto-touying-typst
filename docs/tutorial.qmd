...
`bamboo` (custom), and `matcha` (Universe) decks on the [Gallery](gallery.qmd)
page are live examples.

A theme pinned to an older Touying version usually still works. If it hard-codes
a font you don't have installed, fetch that font with `brand` and, where the
theme takes a font option, point that option at it -- the `clean` deck does the
former, pulling Roboto and Fira Code from Google so the deck renders the same on
any machine. A theme's options are set by rebinding it before use, e.g.
`#let clean-theme = clean-theme.with(...)`. When a theme package exports its own
`.button` / `.fg` / `.bg` helpers (as `clean` does), importing it with `: *`
makes those take over from the base versions, so they match the theme
automatically.
