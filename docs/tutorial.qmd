...
in each — on the [Gallery](gallery.qmd) page.
...
Colours (`accent` / `accent2` / `jet`) and the `mainfont` (body) / `monofont`
(code) fonts apply to every theme; `sansfont`, `fontsize`, and heading weight
style the `clean` theme.
...
`monospace` set the body and code fonts (all for every theme), while
`brand.typography.headings` and weight style the `clean` theme:
...
colour palette, exactly like the built-in themes. The `bamboo` (custom) and
`flow` (Universe) decks on the [Gallery](gallery.qmd) page are live examples.
...
font (`flow-theme.with(code-font: "JetBrains Mono")`).
...
All of these work in every theme. `.alert`, `.button`, and `.small-cite` pick
up the active theme's colours; `.fg` / `.bg` use a fixed accent colour by
default, so pass `options` to choose your own (e.g.
`[text]{.fg options='fill: rgb("#5D639E")'}`).
